...
export class Codegenerator : public I_ast_visitor {
   public:
    Codegenerator(std::string_view name, std::ostream& out,
                  std::string asked_triple = llvm::sys::getDefaultTargetTriple())
        ...
        if (DL.getPointerSize() != 4)
            throw Codegen_exception{std::format("Target pointer size is not 4 bytes but {}", DL.getPointerSize())};
    }
    ...
    llvm::Type* llvm_type(Token_type type) { return llvm_type(type_mapping(type)); }
    ...
    llvm::GlobalValue::LinkageTypes llvm_linkage(Linkage_type t) {
        switch (t) {
            case Linkage_type::EXPORT:
                return llvm::Function::ExternalLinkage;
            case Linkage_type::EXTERN:
                return llvm::Function::ExternalWeakLinkage;
            case Linkage_type::NONE:
                return llvm::Function::PrivateLinkage;
            default:
                throw std::logic_error{"Invalid linkage type"};
        }
    }
    ...
    void* visit(Unary_operation& node) override {
        ...
        switch (node.operation.type) {
            ...
            case Token_type::OP_ADRESS:
                return val;
            default:
                throw std::logic_error{"Invalid unary operation type"};
        }
    }
    ...
    void* visit(Function_call& node) override {
        ...
        if (auto* v = std::get_if<std::unique_ptr<Variable>>(&node.function_address)) {
            ...
            // function = cast2llvmv
            // std::get_if<std::unique_ptr<Variable>>(&node.function_address)->get()->accept(*this);
            throw std::logic_error{"Indirect calls are not implemented yet"};  // TODO
        }

        auto llvm_args_r =
            node.arguments | rnv::transform([this](auto&& arg) { return cast2llvmv arg->accept(*this); });
        std::vector llvm_args{std::from_range, llvm_args_r};
        return builder.CreateCall(function, llvm_args);
    }

    void* visit(Integer_literal& node) override { return builder.getInt32(node.value); }
    ...
    void* visit(Char_literal& node) override { return builder.getInt8(node.value); }
    ...
    void* visit(Variable_declaration_statement& node) override {
        ...
        if (current_function) {
            ...
            llvm::Constant* zero =
                (node.linkage != Linkage_type::EXTERN) ? llvm::ConstantInt::getNullValue(llvm_t) : nullptr;
            llvm::Value* var =
                new llvm::GlobalVariable{module, llvm_t, false, llvm_linkage(node.linkage), zero, var_name};
    ...
    void* visit(Function_definition& node) override {
        ...
        current_function =
            llvm::Function::Create(func_type, llvm_linkage(node.var.linkage), node.var.name.content_str(), &module);
